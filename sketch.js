...
function preload(){
  ...
  collidedSound = loadSound("/audio/collided.wav")
  
  backgroundImg = loadImage("/img/backgroundImg.png")
  sunAnimation = loadImage("/img/sun.png");
  
  trex_running = loadAnimation("/img/trex_2.png","/img/trex_1.png","/img/trex_3.png");
...
function setup() {
  ...
  sun = createSprite(width-100,80,10,10);
...
function draw() {
  ...
  if (gameState===PLAY){
    ...
    if((touches.length > 0 || keyDown("SPACE")) && trex.y  >= height-113) {
      jumpSound.play( )
      trex.velocityY = -12;
       touches = [];
    }
...
function spawnObstacles() {
  if(frameCount % 60 === 0) {
    var obstacle = createSprite(700,height-100,20,30);
